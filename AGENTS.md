...
## Onboarding Fork Collaboration

- Working fork: `https://github.com/0xkydo/d-inference.git`. Both collaborators currently use the GitHub account `0xkydo`; GitHub authentication is separate from local commit author configuration.
- Start each collaborator's separate branch from `codex/cli-onboarding-m3-local`. Use separate clones/worktrees when sharing a Mac, preserve others' changes, and do not work directly on the shared base branch.
- Push to the `0xkydo` fork and open PRs against its `codex/cli-onboarding-m3-local` branch. This checkout calls the fork remote `fork` and upstream `origin`; a fresh clone of the fork normally calls it `origin`. Check remote URLs before pushing. Upstream `Layr-Labs/d-inference` is not the default push or PR destination for this collaboration.
- Read `docs/developer/onboarding-test.md`. Scope: Install → enrollment → account linkage → model selection/download → final Enter-to-start prompt. Require Enter before opening enrollment or the login browser; use neutral styling and selective bold, green success and red errors, without blue/cyan accents.
- Use total physical RAM plus existing safeguards for model-fit estimates; support multiple downloads, hide empty Downloaded sections, and keep additional models expandable. Reuse shared enrollment/login/download code and preserve the update path for completed installs.
- The human tester runs all installer, reset, enrollment, login, and start commands and approves interactive prompts. Agents prepare builds, inspect logs, and fix/test code; never advance the tester's prompts. Include a reset command alongside each test command, with its deletion scope clear. The M3's personal `darkbloom-reset-all` shortcut removes shared model downloads and installers; it is not installed on other machines automatically.
- Production enrollment/account/catalog services at `https://api.darkbloom.dev` are approved for this personal test. Publishing or registering releases, infrastructure deployments, and fleet changes require separate authorization. Local builds test UI only; signed identity acceptance and device trust require separate verification. For UI passes, quit with `q` at the final Enter-to-start prompt.
